...
### Direct Agent API
This release removes the external MCP transport layer entirely. The addon no longer starts an MCP, HTTP, stdio, or JSON-RPC server. Instead, an AI coding agent running inside the Godot editor can call the registered tools directly through the plugin registry.
