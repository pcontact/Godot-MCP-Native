...
func before_each():
	...
	_remove_storage_file("agent_tool_state.cfg")
	_remove_storage_file("mcp_tool_state.cfg")
...
func _remove_storage_file(file_name: String) -> void:
	var original_name: String = _state_manager.config_file_name
	_state_manager.config_file_name = file_name
	var path: String = _state_manager.get_storage_path()
	if FileAccess.file_exists(path):
		DirAccess.remove_absolute(path)
	_state_manager.config_file_name = original_name
...
func test_apply_states_to_registry():
	_registry.register_tool("test_tool_1", "Test 1", {"type": "object"}, func(args): return {})
	_registry.register_tool("test_tool_2", "Test 2", {"type": "object"}, func(args): return {})

	var states: Dictionary = {
		"test_tool_1": false,
		"test_tool_2": true
	}
	_state_manager.apply_states_to_registry(_registry, states)

	assert_true(_registry.has_tool("test_tool_1"), "Tool should still exist")
	assert_true(_registry.has_tool("test_tool_2"), "Tool should still exist")

	var tools: Array = _registry.get_registered_tools()
	for t in tools:
...
func test_apply_states_ignores_unregistered_tools():
	_registry.register_tool("real_tool", "Real", {"type": "object"}, func(args): return {})
	var states: Dictionary = {
		"real_tool": false,
		"fake_tool": true
	}
...
func test_capture_states_from_registry():
	_registry.register_tool("tool_a", "A", {"type": "object"}, func(args): return {})
	_registry.register_tool("tool_b", "B", {"type": "object"}, func(args): return {})
	_registry.set_tool_enabled("tool_a", false)

	var captured: Dictionary = _state_manager.capture_states_from_registry(_registry)
...
func test_validate_core_tool_limit():
	_registry.register_tool("tool", "Test", {"type": "object"}, func(args): return {})
	...
	assert_eq(result["core_limit"], 30, "Core limit should be 30")

func test_get_storage_path():
	var path: String = _state_manager.get_storage_path()
	assert_true(path.ends_with("agent_tool_state.cfg"), "Path should end with agent_tool_state.cfg")
...
func test_config_file_name_constant():
	assert_eq(_state_manager.config_file_name, "agent_tool_state.cfg", "Config file name should be agent_tool_state.cfg")
